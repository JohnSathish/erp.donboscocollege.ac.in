...
import { apiResultFormat, language, pageSelection } from '../../../../shared/model/pages.model';
...
@Component({
    ...
    imports: [CustomPaginationComponent,CommonModule,FormsModule,MatSortModule,SelectModule,DateRangePickerComponent]
})
export class LanguageSettingsComponent {
  ...
  initChecked = false;
  public pageSize = 10;
  public tableData: Array<language> = [];
  public tableDataCopy: Array<language> = [];
  public actualData: Array<language> = [];
  ...
  constructor(
    ...
    this.data.getLanguageData().subscribe((apiRes: apiResultFormat) => {
      this.actualData = apiRes.data;
      this.pagination.tablePageSize.subscribe((res: tablePageSize) => {
        if (this.router.url == this.routes.language) {
  ...
  private getTableData(pageOption: pageSelection): void {
    this.data.getLanguageData().subscribe((apiRes: apiResultFormat) => {
      ...
      apiRes.data.map((res: language, index: number) => {
      ...
      this.dataSource = new MatTableDataSource<language>(this.actualData);
